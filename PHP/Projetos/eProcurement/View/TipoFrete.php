...
if (isset($_POST['Cadastrar'])) {
    if ($_REQUEST['Acao'] == "Inserir") {
        $ObjTipoFrete = buscaTipoFretePorCodErp($_REQUEST[TipoFrete::_COD]);

        if ($ObjTipoFrete->getId() != "" && !is_null($ObjTipoFrete->getId())) {
            $ObjFuncionalidades->ExibeMensagem("Cod erp ja cadastrado para outro tipo de Frete!");
            $ObjFuncionalidades->VoltarPaginaAnterior();
        } else {
            $id = CadastraTipoFrete($_REQUEST);
        }

        $ObjFuncionalidades->Redirecionar("TipoFrete.php");
...
if ($Acao == "Deletar") {
    ...
    $Cadastrados = ContarPedidoPorTipoFrete($_REQUEST['id']);
    if ($Cadastrados == 0) {
        $id = deletarTipoFrete($_REQUEST);
        $ObjFuncionalidades->Redirecionar("TipoFrete.php");
    } else {
        $ObjFuncionalidades->ExibeMensagem("Existem registros vinculados ao tipo de frete, por tanto não é possivel exclui-lo");
        $ObjFuncionalidades->Redirecionar("TipoFrete.php");
    }
}
?>
                        ...
                        <?PHP if ($Acao == "") { ?>
                            <div class="col-lg-12">
                                <div class="panel panel-default">
                                    <form id="carregar" action="TipoFrete.php" name="carregar" method="post">
                                        <div class="panel-heading">
                                            <div class="row">
                                                <div class="col-lg-1">
                                                    <?PHP if ($Permissoes['PTC0005']['INCLUIR'] == 1) { ?>
                                                        <a class="btn btn-primary" data-toggle="modal" href="TipoFrete.php?Acao=Inserir"><i class="fa fa-file-o"></i> Cadastrar</a>
                                                    <?PHP } ?>
                                                </div>
                                                <div class="col-lg-3">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        if (isset($_REQUEST['Filtrar'])) {
                                            ...
                                            $Indice = TipoFrete::_ID;
                                            //Define a tabela que trabalharemos
                                            $Tabela = TipoFrete::_TABLENAME;
                                            ...
                                            $Campos = TipoFrete::_ID . SEPARADOR . TipoFrete::_DESCRICAO . SEPARADOR . TipoFrete::_COD;
                                            //Montamos um array pra fazer a exibição dos resultados em tabela
                                            $Grid = ['Código ERP' => TipoFrete::_COD, 'Descrição' => TipoFrete::_DESCRICAO,];
                                            //Criamos uma condição where padrão para as pesquisas de tabela
                                            $Where = " ORDER BY " . TipoFrete::_COD . " ASC ";
                                            ...
                                            if ($_REQUEST['filtroPorCod'] != "") {
                                                ...
                                                if ($SeqFiltro > 0) {
                                                    $Filtro .= " AND (" . TipoFrete::_COD . " = '" . strtoupper($_REQUEST['filtroPorCod']) . "')";
                                                    $SeqFiltro++;
                                                } else {
                                                    $Filtro .= " (" . TipoFrete::_COD . " = '" . strtoupper($_REQUEST['filtroPorCod']) . "')";
                                                    $SeqFiltro++;
                                                }
                                            }
                                            ...
                                            if ($_REQUEST['filtroPorDesc'] != "") {
                                                ...
                                                if ($SeqFiltro > 0) {
                                                    $Filtro .= " AND (" . TipoFrete::_DESCRICAO . " LIKE '%" . strtoupper($_REQUEST['filtroPorDesc']) . "%')";
                                                    $SeqFiltro++;
                                                } else {
                                                    $Filtro .= " (" . TipoFrete::_DESCRICAO . " LIKE '%" . strtoupper($_REQUEST['filtroPorDesc']) . "%')";
                                                    $SeqFiltro++;
                                                }
                                            }
 ...
 echo "<div class=\"adv-table\" style='overflow: visible;overflow-x: scroll;'>";
                                            if (isset($Consultar) && !empty($Consultar) && ($Consultar)) {
                                                ...
                                                foreach ($Consultar as $value) {
                                                    ...
                                                    if ($Permissoes['PTC0005']['LIXEIRA'] == 1) {
                                                        $str_deletar = <<<EOT
<a class="btn btn-danger btn-xs excluiritem" onclick="javascript: if (!confirm('Você realmente deseja excluir este registro?')){return false;}"  href="TipoFrete.php?Acao=Deletar&id={$value[$Indice]}" id="{$value[$Indice]}"><i class="fa fa-trash-o"></i> Excluir</a>
EOT;
                                                    }
                            ...
                            if ($Acao == "Atualizar") {
                                $ObjTipoFrete = buscaTipoFretePorId($_REQUEST['id']);
                            }
                            ?>
                            <form id="carregar" action="TipoFrete.php" name="carregar" method="post">
                                <div>
                                                        ...
                                                        <div class="col-lg-12">
                                                            <div class="row">
                                                                <div class="col-lg-3">
                                                                    ...
                                                                    <input type="hidden" name="<?PHP echo TipoFrete::_ID; ?>" class="form-control m-bot15" id="<?PHP echo TipoFrete::_ID; ?>" value="<?PHP echo $ObjTipoFrete->getId(); ?>" />
                                                                    <input required="" class="form-control m-bot15" type="text" name="<?PHP echo TipoFrete::_COD; ?>" id="<?PHP echo TipoFrete::_COD; ?>" value="<?PHP echo $ObjTipoFrete->getCod(); ?>" />
                                                                </div>
                                                                <div class="col-lg-9">
                                                                    <label> Descrição </label>
                                                                    <input required="" class="form-control m-bot15" type="text" name="<?PHP echo TipoFrete::_DESCRICAO; ?>" id="<?PHP echo TipoFrete::_DESCRICAO; ?>" value="<?PHP echo $ObjTipoFrete->getDescricao(); ?>"  />
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <div class="col-lg-12">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-lg-3">
                                                                    <label>   </label>
                                                                    <?PHP if (($Acao == "Atualizar" && $Permissoes['PTC0005']['ALTERAR'] == 1) || ($Acao == "Inserir" && $Permissoes['PTC0005']['INCLUIR'] == 1)) { ?>
                                                                        <input type="submit" name="Cadastrar" id="Cadastrar" class="btn btn-info"  value="Confirmar" />    <?PHP } ?><a class="btn btn-info" href="TipoFrete.php">Voltar</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
